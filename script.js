    ...
    let allPosts = []; // Store all posts to filter later

async function loadPosts() {
    const response = await fetch('/posts');
    allPosts = await response.json(); // Store posts in the global variable
...
function displayPosts(posts) {
    ...
    const filterPost = posts.filter((post) => (post == "") )
    const isAuthenticated = localStorage.getItem('token'); // Check if user is logged in
    const currentUserId = getCurrentUserId(); // Get the current user's ID from the token or storage
...
// Add an event listener for when the user types in the browser's search bar
window.addEventListener('keydown', (event) => {
    if (event.key === 'f') { // When the user presses 'f'
        setTimeout(() => {
            const searchKeyword = prompt('Enter a keyword to search for:');
            if (searchKeyword) {
                filterPosts(searchKeyword); // Call filter function
            }
        }, 100); // Delay to ensure prompt appears after the 'Find' command
    }
});
